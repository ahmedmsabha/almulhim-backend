...
- **Content-based subscriptions:** Plans grant explicit unit sets via `plan_units`, with per-region prices (`priceGaza` / `priceWestBank`) and a fixed `accessEndsAt` (not N days from approval). Unique open row is per user+plan. `GET /subscriptions/me` returns `{ subscriptions, overallStatus, entitledUnitIds }`. Lesson `isLocked` is unit-scoped (`preview` always open; `subscriber_only` if `unitId` is in the union of active unexpired plan units). `startsAt` is display-only.
